...
describe('espn normalizer edge cases', () => {
  ...
  it('falls back to an empty status object when the event carries no status', async () => {
    ...
    const g = await one({
      ...
      competitions: [
        {
          competitors: [
            { homeAway: 'home', score: { value: 110 } },
            { homeAway: 'away', score: { value: 101 } },
          ],
        },
      ],
    })
  ...
  it('treats a null score value as no score', async () => {
    const g = await one({
      id: 'nullscore',
      competitions: [
        {
  ...
  it('tolerates a fulfilled response with no events array', async () => {
...
describe('alerts detection edge cases', () => {
  const g = (over = {}) => ({ id: 'g1', home: 'MIN', away: 'BOS', seasonType: 'regular', ...over })

  it('handles a scoreless previous snapshot when a game turns into a nailbiter', () => {
  ...
  it('treats a missing period as pre-regulation, so nothing fires', () => {
...
describe('fetchPlayer parsing edge cases', () => {
  ...
  it('degrades to null bio and empty games when every request is not-ok', async () => {
    stub({ ok: false })
  ...
  it('fills bio fields with null when the athlete record is empty', async () => {
  ...
  it('falls back to the overview birthPlace and reads numeric experience', async () => {
    stub({
  ...
  it('maps the game log by labels, defaulting missing columns and metadata', async () => {
    const gamelog = {
      labels: ['MIN', 'PTS'], // REB/AST columns are absent → those stats resolve to null
      events: {
        e1: { gameDate: '2026-01-19', atVs: '@', gameResult: 'W', opponent: { abbreviation: 'PHX' } },
        e3: { gameDate: '2026-01-01' }, // no opponent/atVs/result
      },
      seasonTypes: [
        {}, // no categories
    ...
    expect(games[0].stats).toEqual({ MIN: '34', PTS: '28', REB: null, AST: null })
...
describe('fetchGameSummary parsing edge cases', () => {
  ...
  const edge = () => ({
    boxscore: {
      players: [
        {
          team: { shortDisplayName: 'Celtics' }, // no abbreviation, no displayName
          statistics: [
            {
              keys: ['minutes', 'points'],
  ...
  it('parses a box score full of missing fields without throwing', async () => {
    ...
    const side = box.sides.find((s) => s.name === 'Celtics')
