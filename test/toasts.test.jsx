...
const game = { id: 'g1', home: 'MIN', away: 'LAL', score: [90, 86] }
...
describe('Toasts', () => {
  ...
  it('phrases a tipoff', () => {
    ...
    expect(screen.getByText('Lakers at Timberwolves')).toBeInTheDocument()
  })

  it('phrases a lead change with the new leader', () => {
    render(<Toasts events={[evt({ kind: 'lead-change', leader: 'LAL', margin: 2 })]} />)
    expect(screen.getByText('Lead change')).toBeInTheDocument()
    expect(screen.getByText('Lakers by 2')).toBeInTheDocument()
  })
  ...
  it('phrases a close finish, including a tie', () => {
    ...
    expect(screen.getByText('Timberwolves by 3 in the fourth')).toBeInTheDocument()
  ...
  it('phrases a final as winner-first, regardless of home or away', () => {
    ...
    expect(screen.getByText('Timberwolves 90–86')).toBeInTheDocument()

    // Away winner: the winning score still leads.
    rerender(
      <Toasts
        events={[evt({ kind: 'final', leader: 'LAL', game: { ...game, score: [86, 90] } })]}
      />
    )
    expect(screen.getByText('Lakers 90–86')).toBeInTheDocument()
  })

  it('opens the game when clicked', async () => {
    const onOpen = vi.fn()
    render(<Toasts events={[evt({ kind: 'final', leader: 'MIN' })]} onOpen={onOpen} />)
    await userEvent.click(screen.getByText('Timberwolves 90–86'))
