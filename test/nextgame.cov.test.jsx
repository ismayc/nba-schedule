...
const draw = (games, followed = []) => {
  localStorage.setItem('nba:followed', JSON.stringify(followed))
  return render(
    <FollowProvider>
...
describe('NextGame', () => {
  ...
  it('prefers a followed team’s next game over an earlier one', () => {
    draw(
      [
        game({ id: 'early', tip: '2026-07-19T17:00:00.000Z', home: 'LAL', away: 'GS' }),
        game({ id: 'mine', tip: '2026-07-19T23:00:00.000Z', home: 'NY', away: 'BOS' }),
      ],
      ['NY']
    )
  ...
  it('stacks every game sharing the earliest tip when no team is followed', () => {
    draw([
      game({ id: 'a', home: 'NY', away: 'BOS' }),
      game({ id: 'b', home: 'LAL', away: 'GS' }),
      game({ id: 'c', tip: '2026-07-19T23:00:00.000Z', home: 'DAL', away: 'PHX' }),
    ])
    ...
    expect(screen.queryByText('Suns')).toBeNull()
  ...
  it('a live game takes over from an upcoming one', () => {
    draw([
      ...
      game({ id: 'later', tip: '2026-07-19T23:00:00.000Z', home: 'LAL', away: 'GS' }),
    ])
  ...
  it('stacks several live games, with a period badge on each', () => {
    const { container } = draw([
      ...
      game({ id: 'l2', live: true, period: 4, statusLabel: 'Q4 1:02', home: 'LAL', away: 'GS' }),
    ])
  ...
  it('a followed team playing live wins outright over the other live games', () => {
    draw(
      [
        game({ id: 'l1', live: true, period: 1, home: 'LAL', away: 'GS' }),
        game({ id: 'mine', live: true, period: 2, home: 'NY', away: 'BOS' }),
      ],
      ['BOS']
    )
    expect(screen.getByText('🔴 Live now')).toBeInTheDocument()
