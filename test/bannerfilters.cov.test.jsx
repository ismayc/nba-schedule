...
// choice to make.
const NOW = new Date('2025-10-21T20:00:00.000Z')
...
const chooseServices = (keys) => {
  localStorage.setItem('nba:services', JSON.stringify(keys))
...
describe('the Next game banner and the services filter', () => {
  it('counts down to the true next game when no services filter is on', async () => {
    ...
    expect(banner().textContent).toContain('Thunder')
    expect(banner().textContent).not.toContain('Lakers')
  })

  it('skips a game the viewer cannot watch and counts down to the next one they can', async () => {
    // Peacock only -- the earlier game is on NBA TV, which no chosen service carries.
    ...
    expect(banner().textContent).toContain('Lakers')
    expect(banner().textContent).not.toContain('Thunder')
  })
  ...
  it('offers no game at all when nothing on the schedule is watchable', async () => {
    // Prime Video carries neither NBA TV nor Peacock.
    chooseServices(['prime'])
    await mount()
