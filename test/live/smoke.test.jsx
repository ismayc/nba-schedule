...
import { playoffRace } from '../../src/utils/standings.js'
...
//
// The main suite used to do this job by accident: 27 of its test files rendered the live
...
describe('every game and every team opens', () => {
  ...
  it('renders the team panel for every team', () => {
    // Derived once and passed in, as App does. TeamPanel would otherwise run the whole
    // playoff race once per team. It also reads the follow context, whose default is
    // null, so it needs FollowProvider where the WNBA sibling's panel did not.
    const race = playoffRace(GAMES)
    for (const t of TEAMS) {
      const { container, unmount } = render(
        <FollowProvider>
          <ServicesProvider>
            <TeamPanel abbr={t.abbr} games={GAMES} race={race} tz="America/New_York" onClose={() => {}} />
          </ServicesProvider>
        </FollowProvider>
      )
