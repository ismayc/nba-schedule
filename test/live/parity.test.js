...
describe('frozen stand-ins keep the shape of the live modules', () => {
  ...
  // not mistakes (the first playoff game carries a round and a series the preseason board
  ...
  // Only once the table has rows. The player table is EMPTY from the rollover until
  // opening night (it was on September 19, 2026, when this was written), and an empty
  // table has no fields to check. Asserting them unconditionally would fail every
  // preseason refresh, which is exactly the false alarm this suite must never raise.
