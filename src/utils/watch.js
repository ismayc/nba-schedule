...
//
// Regional/local feeds (RSNs, market TV stations) can't be folded into a bundle's
// mapping — carriage is market-dependent, so a single answer would be wrong (a
// Portland YouTube TV carries that market's station; an Atlanta one doesn't).
// Instead the picker offers every local/regional feed the schedule data names, and
// the viewer checks the ones their own provider carries. See LOCAL_CATALOG below.
...
export const SERVICE_CATALOG = [
  ...
  { key: 'leaguepass', label: 'NBA League Pass', kind: 'stream', match: carries('NBA League Pass') },
...
// service matches it directly (it's US-national cable, not a local feed).
