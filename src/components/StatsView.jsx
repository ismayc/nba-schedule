...
// manufactured ties: 16 across the five 2025-26 per-game top tens, against two at 2dp.
...
// names five games and then can't show you any of them is a dead end.
...
// midseason trade reads as the two clubs it was rather than as whoever holds his rights
...
function SeasonTeams({ teams, onPickTeam }) {
  return (
    <span className="lead-teams">
      {teams.map((t, i) => (
        <Fragment key={t.abbr}>
          {i > 0 && <i className="lead-arrow" aria-hidden="true">→</i>}
...
// fetch-history built with this same leaderboard(), ties and qualifiers included). Both
// carry season-accurate `teams`, so archived boards badge their rows too — they did not
// before, because ESPN's per-athlete stats answer with the player's CURRENT club even when
// an older season is asked for, and only for players who later moved, which left a
// historical board mixing correct and anachronistic badges.
export function Leaders({ getRows, onPickTeam, onPickPlayer }) {
  ...
  return (
    <div className="card">
      ...
      <p className="fine">
        Qualification follows the NBA&apos;s published minimums for a full season — 58 games
        for a per-game average, 300 made field goals for FG%, 82 made threes for 3P% — each
        scaled to how much of the season has been played, so a board mid-season ranks who
        has actually been available rather than sitting empty. Ties share a rank; a traded
        player shows every team he played for, oldest first.
      </p>
    </div>
  )
}
