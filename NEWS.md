...
- **Fixed the squished "Next up" rows in a team's Standings pop-out.** The `.drill`
  ...
  date column and the right-aligned tipoff time both no-opped and the row squished to
...
- **Regional feeds show their full name.** ESPN emits some regional sports networks as terse
  ...
  the local-channel picker now maps the acronym RSNs to their full names (Bulls' CHSN,
  Pelicans' GCSEN, Wizards' MNMT, Knicks' MSG, Nets' YES, Blazers' ROOT, and their overflow
  feeds), while keeping the raw code as the entry key and match target so saved picks and
  game matching are unchanged. TV-station callsigns ("KFAA-TV") are left as ESPN emits them,
  since that is how the station is known; an unmapped string still falls through to the raw
  value. Add to the map as new RSNs appear.
...
- **Repo-level guards now run in the test suite.** New `test/guards.test.js`, ported from
  ...
  app's `nba:` prefix and never a sibling's, because the hub and all eleven viewers are
...
- **The data scripts now fetch from `site.web.api.espn.com`.** ESPN's edge started
  ...
  every unattended refresh: the WNBA viewer's had been failing with `HTTP 403` all
  day, and this repo's own season watch was hitting the same wall (silently — see
  below). The same URLs answer normally from a home connection, so the block is on
  the host, not on us. Its sibling `site.web.api` carries the identical routes with
  identical payloads and no block, verified route by route.
- Nothing about the app changed — same data, same season, same tests. The live
  score overlay was never affected, because it runs in your browser rather than in
  a datacenter.
