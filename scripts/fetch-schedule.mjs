...
//
// NBA seasons span two calendar years; ESPN's `season` is the ENDING year:
//   season=2026 → the 2025-26 season · season=2027 → the (unposted) 2026-27 season.
//
...
// knockout quarter/semifinals — is an ordinary regular-season game, so only the
// championship is reclassified. (Careful: "Quarterfinals"/"Semifinals" contain the
// substring "final", so the match must require the word "championship".)
const CUP_FINAL = /(nba cup|in-season tournament).*championship/i
...
//
// This is the failure the floor could not see. On 2026-08-26 ESPN's 2026-27 team list
// dropped from 30 teams to 13, and grew a "LON" (the London Lions, a preseason
// exhibition opponent, not a franchise). That truncation happened to gut the schedule
// far enough to trip the 0.9 floor, but it need not have: 28 teams still yields ~1,150
// games against a floor of 1,080, so a milder ESPN edit would have quietly published a
// roster missing two franchises. Every team's schedule, logo, and leaderboard entry
// hangs off this list, so it gets checked exactly, and checked first, before ~90
// doomed HTTP calls.
//
// Real roster changes do happen (expansion, relocation), so `--allow-roster-change`
// overrides it for the one run that lands them.
...
export async function fetchTeams(season = SEASON) {
  ...
  return teams
    .map((t) => ({
      ...
      name: t.name, // "Celtics"
      location: t.location, // "Boston"
      displayName: t.displayName, // "Boston Celtics"
      color: `#${t.color}`,
...
// Playoff series metadata lives in an unstructured headline: "West 1st Round - Game 1",
// "NBA Finals - Game 3". Parse it into { round, game } so the bracket can group games
// into series without guessing from dates. "Semifinals" and a conference "…Finals" both
// contain "final", and a conference final must not be read as the championship, so order
// matters: play-in, first round, semifinals, NBA Finals, then the generic conf. finals.
const ROUND_PATTERNS = [
  [/play[-\s]?in/i, 'PI'],
  [/1st\s*round|first\s*round/i, 'R1'],
  [/semifinal/i, 'CSF'],
  [/nba\s*final/i, 'Final'],
  [/final/i, 'CF'],
]
...
// The All-Star events live on the scoreboard, not in any team's schedule — the drafted
// sides aren't franchises, so a per-team fetch never sees them (2026's format is a
// four-game mini-tournament, all competition type ALLSTAR). The window is the mid-Feb
// All-Star break, NOT the whole month: a full NBA February is ~200 events and the
// scoreboard silently truncates at `limit`, which dropped the All-Star weekend
// entirely on a month-wide query (verified live). Their teams have no logos and
// ALL-CAPS names, so carry tidy display names on the game for the UI, which can't
// look them up in TEAMS.
const titleCase = (s) => s.toLowerCase().replace(/\b\w/g, (m) => m.toUpperCase())

async function fetchAllStar() {
  const d = await getJson(`${SITE}/scoreboard?dates=${SEASON}0210-${SEASON}0225&limit=300`)
  return (d.events || [])
...
// Basketball has no analogue to enumerating goals — a game has ~50 scoring plays and
// the season has ~60,000, so per-basket events are neither fetchable nor useful. The
...
// RANGE, so a month per request covers the season in ~7 calls.
...
async function enrichWithBoxScores(games) {
  ...
  for (const ym of months) {
    const d = await getJson(`${SITE}/scoreboard?dates=${monthRange(ym)}&limit=400`)
...
async function main() {
  console.log(`Fetching ${SEASON} NBA teams…`)
